...
- **FORWARD TEST LANCE — 2026-08-20 15:19 (heure serveur), deploye et prouve par la session Infrastructure** (rapport inter-session recu le jour meme, tout MESURE) : cible `49894cb` atteinte, `PAPER_PARAMS=band=1.5` actif — log de boot `Demarrage : SMA(20/50, marge 1.5x frais)`, premiere decision `TEST : ecart -0.25% vs marge 2.43% -> zone neutre` (ce croisement aurait churne avant). **Baseline A/B gelee** : paper_stats.csv = 3 176 lignes au bascule, equity 6 470,38 $, 78 ordres band=0 en 11 j (frais 4 707 $). Etat conserve (reprise paper_state.json, zero perte). Note de provenance : le serveur ne partait pas de `f0fdc81` mais de `25bc668` (maj intermediaire du 18/08 par l'infra a `348b414`, puis un pull manuel — le depot serveur peut bouger hors des passages de l'infra ; les briefs marquent desormais la baseline serveur SUPPOSEE, seul l'etat rapporte par l'infra fait foi).
