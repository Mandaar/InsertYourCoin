...
| BUG-005 | 2026-06-08 | P1 | Walk-forward : warm-up des indicateurs perdu a chaque fenetre OOS -> classements fausses (SMA 200 amputee) ; **contamine le verdict Etape 1** | Indicateurs recalcules sur chaque tranche isolee | A FAIRE : indicateurs sur df complet + slice avec warm-up, compter a partir de `test_start` (`optimizer.py:100`, `backtester.py:101`) | A AJOUTER (test walk-forward warm-up) | AUDIT_MOTEUR B1 | **Ouvert** |
| BUG-006 | 2026-06-08 | P1 | optimizer selectionne sur metrique non bornee (flat=0 bat les negatives ; `inf` gagne) -> biais de selection (inaction presentee "robuste") | `_best_on` argmax sans garde (`optimizer.py:45,51`) | A FAIRE : exiger `n_trades>=5-10`, exclure `inf`, `NaN` pour metriques degenerees | A AJOUTER | AUDIT_MOTEUR B2 | **Ouvert** |
| BUG-007 | 2026-06-08 | P2 | `optimizer.py` (le "juge" du projet) sans AUCUN test de non-regression | aucun `test_optimizer.py` | A FAIRE : `tests/test_optimizer.py` (non-chevauchement fenetres, reproductibilite, garde donnees) | A AJOUTER | AUDIT_MOTEUR B3 | **Ouvert** |
